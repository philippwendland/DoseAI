...
#Adaptation from https://github.com/Valentyn1997/CausalTransformer/blob/main/src/data/mimic_iii/semi_synthetic_dataset.py
# Valentyn Melnychuk et al., "Causal Transformer for Estimating Counterfactual Outcomes" ,  https://doi.org/10.48550/arXiv.2204.07258
# in Journal reference: Proceedings of the 39-th International Conference on Machine Learning, Baltimore, Maryland, USA, PMLR 162, 2022
...
ROOT_PATH = '/Users/jaschob/Documents/GitHub/Promotion1_Jennifer/SemiSynth_MIMIC4'
from load_data_mimic4 import load_mimic4_data_raw
from utils_mimic4 import sigmoid, SplineTrendsMixture
from real_dataset_mimic4 import MIMIC4RealDataset
from dataset_collection_mimic4 import SyntheticDatasetCollection
...
class SyntheticOutcomeGenerator:
    ...
    def simulate_untreated(self, all_vitals_in: pd.DataFrame, static_features: pd.DataFrame):
        """
        ...
        all_vitals = all_vitals_in.copy(deep=True)#new damit die urspruenglichen Daten nicht ueberschrieben werden
...
class SyntheticTreatment:
    ...
    # def treatment_proba_new(self, patient_df, t):
    #     """
    #     Calculates propensity score for patient_df and time-step t
    #     Args:
    #         patient_df: DataFrame of ONE patient with MultiIndex ['subject_id', 'hours_in']
    #         t: Time-step
    ...
    #     # Zugriff auf Zeitfenster via second level index ('hours_in')
    #     # Annahme: patient_df enthält NUR einen subject_id
    #     avg_y = patient_df.loc[(slice(None), agr_range), self.confounder_outcomes].values.mean()##
        
    #     x = patient_df.loc[(slice(None), t), self.confounding_vars].values.reshape(1, -1)##
    #     f_x = self.confounding_dependency(x)
...
class MIMIC4SyntheticDataset(MIMIC4RealDataset):
    """
    Pytorch-style semi-synthetic MIMIC-4 dataset
    """
    def __init__(self,
...
class MIMIC4SyntheticDatasetCollection(SyntheticDatasetCollection):
    """
    ...
    def __init__(self,
        ...
        """
        super(MIMIC4SyntheticDatasetCollection, self).__init__()
        self.seed = seed
        np.random.seed(seed)
        #neu, all_variabels und static_features direkt übergeben
        if x_train == None:
            print(1)
            all_vitals, static_features = load_mimic4_data_raw(ROOT_PATH + '/' + path, min_seq_length=min_seq_length,
            ...
            all_vitals_train, static_features_train = x_train, x_train
            all_vitals_test, static_features_test = x_test, x_test
            all_vitals_val, static_features_val = x_val, x_val

        self.train_f = \
            MIMIC4SyntheticDataset(all_vitals_train, static_features_train, synth_outcomes_list, synth_treatments_list,
                                   treatment_outcomes_influence, 'train')
        #self.train_f.plot_timeseries()

        if split['val'] > 0.0:
            self.val_f = MIMIC4SyntheticDataset(all_vitals_val, static_features_val, synth_outcomes_list, synth_treatments_list,
                                                treatment_outcomes_influence, 'val')

        self.test_cf_one_step = \
            MIMIC4SyntheticDataset(all_vitals_test, static_features_test, synth_outcomes_list, synth_treatments_list,
